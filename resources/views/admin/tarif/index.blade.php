 ...
 <!--End Modal-->
     <div class="card shadow mb-4">
         <div class="card-header d-sm-flex align-items-center justify-content-between py-3">
             ...
             <a href="{{route('tambah.tarif')}}" class="d-none d-sm-inline-block btn btn-primary btn-sm shadow-sm">
               ...
               <div class="table-responsive">
                 <table class="table table-bordered table-striped" id="dataTable" width="100%" cellspacing="0">
                   ...
                   @foreach ($tarif as $daya)
                     <tr>
                       <td width="10%">{{$daya->no_tarif}}</td>
                       <td>{{ $daya->golongan }}</td>
                       <td>{{$daya->daya }}</td>
                       <td>{{ $daya->tarif_kwh }}</td>
                       <td>
                         <a href ="{{route('edit.tarif', $daya->id)}}" title="Edit" class="btn btn-circle btn-warning">
                             <i class="fas fa-pen"></i>
                         </a>
                         <a href="javascript:;" data-toggle="modal" onclick="deleteData({{$daya->id}})" data-target="#DeleteModal" class="btn btn-circle btn-danger"><i class="fa fa-trash"></i></a>
                       </td>
                     </tr>
                   @endforeach
                   </tbody>
